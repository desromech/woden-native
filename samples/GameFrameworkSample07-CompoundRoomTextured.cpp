...
int woden_main(int argc, const char **argv)
{
    ...
    {
        auto wallMeshBuilder = Woden::Rendering::MeshBuilder();
        auto wallsCompoundShape = std::make_shared<CompoundCollisionShapeComponent> ();

        for(auto &wall : walls)
        {
            wallMeshBuilder    
                .setMaterial(wall.material)
                .addCubeWithBox(wall.box);

            auto collisionShapeComponent = std::make_shared<BoxCollisionShapeComponent> ();
            collisionShapeComponent->halfExtent = wall.box.halfExtent();
            collisionShapeComponent->transform.translation = wall.box.center();
            wallsCompoundShape->children.push_back(collisionShapeComponent);
        }

        // Mesh
        auto actor = MakeActor<Actor> ();
        auto meshComponent = std::make_shared<ActorMeshSceneComponent> ();
        meshComponent->mesh = wallMeshBuilder
            .generateTexcoordsWithFacePlanarTransformWithScale(Vector2(1.0f))
            .finishMesh();
        actor->addComponent(meshComponent);

        // Physics
        actor->addComponent(wallsCompoundShape);
