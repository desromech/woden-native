...
int woden_main(int argc, const char **argv)
{
    ...
    // Falling capsule
    {
        auto shape = std::make_shared<CapsuleYCollisionShape> ();
        shape->setHeightAndRadius(1.0f, 0.3f);

        auto rigidBody = std::make_shared<RigidBody> ();
        rigidBody->shape = shape;
        rigidBody->setMass(1);
        rigidBody->computeMassDistribution();
        rigidBody->setPosition(Vector3(0.6f, 1.5f, 0));
    ...
    // Floor box
    {
        ...
        rigidBody->setMass(0);
        rigidBody->computeMassDistribution();
        rigidBody->setPosition(Vector3(0, -0.25, 0));
