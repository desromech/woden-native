...
#include "Woden/Physics/RodConstraint.hpp"
...
int woden_main(int argc, const char **argv)
{
    ...
    // Rod constraint
    auto rod = std::make_shared<RodConstraint> ();
    rod->firstObject = anchorBox;
    rod->secondObject = suspendedBox;
    rod->secondObjectLocalAnchorPoint = Vector3(-0.5, 0, 0);
    world->addForceGenerator(rod);
